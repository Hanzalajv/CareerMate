...
export function buildRetryPrompt(context, questionText, previousAnswer, questionId, reason) {
  const question = QUESTIONS.find(q => q.id === questionId)

  return `You are CareerMate. The user's answer didn't work.
...
Acknowledge their answer briefly, then gently redirect them to actually answer the question.
Keep it 1-2 sentences. Be encouraging, not frustrated.`
}